...
##
##Checks for something called a "SendPortgroup" in an application named "Biztalk"... :)
###################
$Biztalk_server = "SQLSERVER\INSTANS"
...
$y = 0; [Array] $MyNewObject = $null;
$myObjects=@()
$MyNewObject = foreach($item in ($BTSexp.SendPortGroups))
{
...
$statuscode = 0
$statusOutput = "OK: SendPortGroup $($item.name) is started"
...
if ($myErrorObjects.count -gt 0){
    $statuscode=2
    $statusOutput = "CRITICAL Check these SendPortGroups: "
