...
[string]$folderfilter = "New Folder"
...
##threshold är i utgångsläget 60 minuter
[int]$threshold = 6000
...
$statuscode = 0
$statusOutput = "OK: Folder is within threshold"

$filesWithError = @()

    #Hämta filer i foldern, sorterat på LastWriteTime (nyaste först)
    $fileObjects = Get-ChildItem -Directory -Path $folderpath | Where-Object -FilterScript { $_ -like $folderfilter} | Sort-Object -Property LastWriteTime -Descending
...
if($filesWithError -ne $null){
   $statuscode=2
   $statusOutput = "CRITICAL: $folderpath is older than $threshold seconds: $filesWithError"
